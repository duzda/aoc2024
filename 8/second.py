...
def calculate_new_positions(
    a: Position, b: Position, antenna_map: Map
) -> list[Position]:
    positions: list[Position] = []
    diff = b - a

    while Position.is_valid(a, antenna_map):
        positions.append(a)
        a -= diff

    while Position.is_valid(b, antenna_map):
        positions.append(b)
        b += diff
...
def get_unique_positions(antenna_map: Map, antennas: Antennas) -> set[Position]:
    ...
    for antenna_positions in antennas.values():
        for antenna_a, antenna_b in combinations(antenna_positions, 2):
            for antinode in calculate_new_positions(antenna_a, antenna_b, antenna_map):
